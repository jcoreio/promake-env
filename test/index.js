...
describe('envRule', function () {
  ...
  it('works', async function (): Promise<void> {
    await exec(`babel-node ${promake} clean server client`)
    ...
    await exec(`babel-node ${promake} server client`)
    expect(await getMTime(buildDir)).to.equal(firstBuildTime)

    await exec(`babel-node ${promake} server client`, {
      env: { ...process.env, FOO: 1 },
    })
  ...
  it('replaces invalid JSON', async function (): Promise<void> {
    ...
    await exec(`babel-node ${promake} server`)
    const newData = JSON.parse(
      await promisify(fs.readFile)(serverEnvFile, 'utf8')
    )
